  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden h-44 sm:h-52">
        ...
        <img
          src={experience.images?.[0] || "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=800"}
        ...
        <div className="absolute top-3 left-3 z-20 flex flex-wrap gap-1.5">
          {experience.tags?.filter((t) => !t.startsWith("mkt:")).slice(0, 2).map((tag) => (
      ...
      <div className="p-4 sm:p-5">
        <h3 className="font-serif font-bold text-lg text-card-foreground group-hover:text-primary transition-colors mb-1">
          {emoji} {experience.trip_name}
        </h3>

        {experience.summary && (
          <p className="text-sm text-muted-foreground line-clamp-2 mb-3">{experience.summary}</p>
        )}
        ...
        <div className="flex items-end justify-between pt-3 border-t border-border gap-2">
          <div>
            <div className="flex items-baseline gap-1.5">
              <span className="text-2xl font-bold text-primary">₹{experience.price_default.toLocaleString("en-IN")}</span>
              <span className="text-xs text-muted-foreground">per person</span>
            </div>
          </div>
          {experience.booking_live ? (
